...
public class JFrame extends javax.swing.JFrame {
    ...
    private void CalcButtonActionPerformed(java.awt.event.ActionEvent evt) {                                           
        ...
        TwoThreads one = new TwoThreads(model, "первая нить");
        TwoThreads two = new TwoThreads(model, "вторая нить");
...
//            model.setValueAt(null, j, 3);
        ...
        for (int i = 0; i < countRows; i++) {
            if (i%2==0){
                one.run();
                System.out.println("Работает " + one.name);
            }
            else{
                two.run();
                System.out.println("Работает " + two.name);
            }        
        }
    }                                          
    ...
    public class TwoThreads extends Thread{ 
        ...
        public void run(){
            ...
            for(int j = 0; j < data.size(); j++){
                ...
                model.setValueAt(IntegralFunc, j, 3);
            }   
        }   
    }
